...
export const WorkerVerifyEmail = async (req, res) => {
    try {
...
//Resend Otp
export const WorkerResendOTP = async (req, res) => {
    try {
        ...
        LogError("WorkerResendOTP", error);
        return errors.serverError(res, "Something went wrong");
    }
}

// ForgotPassword
export const WorkerForgotPassword = async (req, res) => {
    try {
        const filteredBody = filterObj(req.body, 'email');
        const { email } = filteredBody;
        ...
        LogError('WorkerForgotPassword', error);
        return errors.serverError(res, 'Something went wrong' + error.message);
    }
}

// Reset Password
export const WorkerResetPassword = async (req, res) => {
    try {
        ...
        LogError("WorkerResetPassword", error);
        return errors.serverError(res, 'Something went wrong' + error.message);
    }
}
...
export const WorkerLogin = async (req, res) => {
    try {
        ...
        `,
                ...
                LogData("WorkerLogin", `Login notification sent to ${user.email}`);
            } catch (err) {
                LogError("WorkerLogin-Notify", err);
